...
describe("SumSquares", () => {
  it("should count the square of the values the list", () => {
    expect(sumSquares([1, 2, 3])).toEqual(14);
    expect(sumSquares([[1, 2], 3])).toEqual(14);
    expect(sumSquares([[[[[[[[[1]]]]]]]]])).toEqual(1);
    expect(sumSquares([10, [[10], 10], [10]])).toEqual(400);
    expect(sumSquares([1, [[3], 10, 5, [2, [3], [4], [5, [6]]]], [10]])).toEqual(325);
  });
});


describe("realSize", function () {
  it("returns the total number of integers, stored inside this array. ", () => {
...
describe("replicate", function () {
  it("returns an array containing repetitions of the number argument.", () => {
    expect(replicate(3, 5)).toEqual([5, 5, 5]);
  });

  it("should return [] if the time is negative", function () {
    expect(replicate(-6, 2)).toEqual([]);
  });
});
